...
class ExcelRequirements105(object):
    """[summary]
    ...
    def __init__(self, requirements_filename=None):
        ...
        op_df = self.load_operating_point(xls_filename=self.req_filename)
        cfg_df = self.load_circuit_cfg(xls_filename=self.req_filename)
    ...
    @staticmethod
    def load_operating_point(xls_filename):
        usecols = ['OPN', 'Vi (V)', 'Vo (V)', 'Po (W)']
        df_op = pd.read_excel(io=xls_filename, sheet_name='OperatingPoints', skiprows=0, usecols=usecols)
        df_op = df_op.rename(index=str, columns={'OPN': 'OPN', 'Vi (V)': 'Vin', 'Vo (V)': 'Vout_ref', 'Po (W)': 'Pout_ref'})
        df_op = df_op.set_index('OPN')
        return df_op

    @staticmethod
    def load_circuit_cfg(xls_filename):
        df_cfg = pd.read_excel(io=xls_filename, sheet_name='CircuitConfigs.', skiprows=0, orient='columns')
        df_cfg = df_cfg.rename(index=str, columns={'fs (Hz)': 'fs', 'Cdclink (F)': 'Cdclink', 'Cdm (F)': 'Cdm'})
    ...
    def load_simuation_list(xls_filename):
        ...
        return pd.read_excel(io=xls_filename, sheet_name='Simulations', header=1, orient='columns', usecols=usecols)
